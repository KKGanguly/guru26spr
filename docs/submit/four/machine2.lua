...
local rpg_rules = {
  idle = {
    action = function(p) print("\n[" .. p.name .. "] is idling. HP: " .. p.hp) end,
    transitions = { walk = "moving", attack = "attacking", hit = "staggered", die = "dead" }
  },
  
  moving = {
    action = function(p) print("[" .. p.name .. "] is walking forward.") end,
  ...
  dead = {
    action = function(p) print("[" .. p.name .. "] has collapsed to the ground.") end,
